...
def word_freq_dict(skip_header):
    """Downloads the text of "The Adventures of Sherlock Holmes" from the Gutenberg Project website, 
    ...
    strippables = string.punctuation + string.whitespace + ''.join(filter(lambda x: not x.isprintable(), string.printable))
    stop_words = ["the", "and", "a", "an", "in", "on", "at", "to", "of", "for", "with", "that", "this", "these", "those", "is", "was", "were", "am", "are", "be"]
    ...
    # each word
        for word in line.split(): 
            clean_word = ''.join(char for char in word.strip(strippables) if char not in string.punctuation) # debug by chatGPT
